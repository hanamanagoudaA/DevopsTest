...
<util:vxml pageName="DtmfMenuPresentation.jsp" cacheable="false" rootRequired="false">
    <!-- model scripts -->
    ...
    <util:ecma id="framework/nardtmfserviceutils.es" />
    ...
    <util:form id="StartDtmfMenuPresentation">
        <dtmf:presentationDtmfMenu id="<%=id%>" stateId="<%=stateID%>" menuOptions="<%=menuOptionsString%>"/>
        <util:block>
    ...
    <util:catch event="event.nuance.dialog">
        ...
        <util:if cond="message == null">
            <util:assign name="message" expr="'A dialog event was thrown while processing dynamic menu dialog.'" />
        </util:if>
    ...
    <util:catch event="connection.disconnect.hangup">
        ...
        <util:assign name="message" expr="_message" />
        <util:if cond="message == null">
            <util:assign name="message" expr="'A hangup event was thrown while processing dynamic menu dialog.'" />
        </util:if>
        <util:return namelist="returnCode modelData event message" />
    </util:catch>
	<util:catch>
		...
		<util:assign name="message" expr="_message" />
		<util:if cond="message == null">
			<util:assign name="message" expr="'An event was thrown while processing dynamic menu dialog.'" />
		</util:if>
        <util:return namelist="returnCode modelData event message" />
	</util:catch>
</util:vxml>
